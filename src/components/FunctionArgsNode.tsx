  ...
  return (
    <div style={{
      ...
      <div style={{ display:"flex", alignItems:"center", gap:7, padding:"8px 10px", background:"#081410", borderBottom:`0.5px solid ${C}33`, borderRadius:"8px 8px 0 0" }}>
        ...
        <span style={{ fontWeight:600, color:C, fontSize:11, letterSpacing:"0.06em" }}>{t("node.function_args", "ARGUMENTS")}</span>
        <span style={{ marginLeft:"auto", fontSize:9, color:`${C}99`, background:`${C}18`, padding:"1px 6px", borderRadius:4, marginRight:4 }}>{t("node.function_args.input", "entrée")}</span>
        <button
          onClick={(e) => {
            e.stopPropagation();
      ...
      <div style={{ padding:"7px 10px 10px", minHeight:28 }}>
        {args.length === 0
          ? <span style={{ fontSize:10, color:"#2a4a30", fontStyle:"italic" }}>{t("node.function_args.none", "aucun argument")}</span>
          : args.map((a, i) => {
